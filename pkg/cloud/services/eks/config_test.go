	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.wantErr {
				...
				g.Expect(kubeconfigSecret.Data[clusterAutoscalerKubeconfigSecretKey]).ToNot(BeEmpty())
				g.Expect(kubeconfigSecret.Data[clusterAutoscalerTokenSecretKey]).ToNot(BeEmpty())
			}
		})
	}
}
