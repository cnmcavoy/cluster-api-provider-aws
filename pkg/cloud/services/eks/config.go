...
const (
	...
	clusterAutoscalerKubeconfigSecretKey = "cluster-autoscaler-value"
	clusterAutoscalerTokenSecretKey      = "cluster-autoscaler-token-file"
)
...
func (s *Service) createCAPIKubeconfigSecret(ctx context.Context, cluster *eks.Cluster, clusterRef *types.NamespacedName) error {
	...
	config.AuthInfos = map[string]*api.AuthInfo{
		userName: {
			TokenFile: "./" + clusterAutoscalerTokenSecretKey,
		},
	}
	...
	secretData[clusterAutoscalerKubeconfigSecretKey] = out
	secretData[clusterAutoscalerTokenSecretKey] = []byte(token)
...
func (s *Service) updateCAPIKubeconfigSecret(ctx context.Context, configSecret *corev1.Secret, cluster *eks.Cluster) error {
	...
	config.AuthInfos = map[string]*api.AuthInfo{
		userName: {
			TokenFile: "./cluster-autoscaler-token-file",
		},
	}
	out, err = clientcmd.Write(*config)
	if err != nil {
	...
	configSecret.Data[clusterAutoscalerKubeconfigSecretKey] = out
	configSecret.Data[clusterAutoscalerTokenSecretKey] = []byte(token)
